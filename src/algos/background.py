...
def preprocess_floats(file_content):
    """
    ...
    base = find_free_placeholder_base(file_content)
    ...
    for m in STRING_RE.finditer(file_content):
        pieces.append(repl_segment(file_content[last_end:m.start()]))
        pieces.append(m.group(0))
        last_end = m.end()

    pieces.append(repl_segment(file_content[last_end:]))
